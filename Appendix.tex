...
\textbf{AnyLogic model.} \texttt{FermaCore\_Replication\_Harness\_Brams\_testV2.0.alp}
is the model used for every result. It implements the full process flow
(Sec.~\ref{sec:conceptual}), press scrap and a selectable dryer-queue rule
(\texttt{queueStrategy}), machine downtime on the blenders and packaging lines,
and a Parameter-Variation experiment (\texttt{ReplicationsExp}) that runs $n=20$
replications and writes one KPI row per replication to CSV. A scenario is run by
setting the \texttt{Main} parameters (e.g.\ \texttt{numDryers},
\texttt{cleaningTeamCapacity}, \texttt{demandMultiplier}, \texttt{pressScrapRate})
...
\begin{itemize}\setlength\itemsep{0pt}
  ...
  \item \texttt{make\_figures.py}: builds the throughput, utilisation and DOE-Pareto figures from \texttt{test\_data/}.
