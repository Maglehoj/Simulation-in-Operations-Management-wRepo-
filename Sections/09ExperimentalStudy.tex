...
Throughput stays flat at $\approx61{,}000$ bottles across the sweep, but for a
plan-fixed run that alone is expected. The informative result is that the
\emph{makespan barely moves} (\SI{440.6}{h} at $\times1.0$ versus \SI{433.6}{h}
at $\times2.0$): doubling the arrival rate does not let the line finish sooner,
which shows the dryer caps the \emph{completion rate} independently of the plan
size, and not merely the batch count. The compressed load is instead absorbed as
congestion: at $\times2.0$ mean flow time rises \SI{11}{\percent}
(\SI{182}{\hour}$\rightarrow$\SI{203}{\hour}) and average WIP rises
\SI{12}{\percent} (19.8$\rightarrow$22.2), while release violations stay at zero,
so the line still honours the one-day rule at a doubled pace. We model the 2030
increase as schedule compression; a direct volume-scaling experiment would
quantify the exact sustainable ceiling, but the rate ceiling is already visible
here, and drying (\SI{95}{\percent}) is the station that would bind first if the
batch count itself grew.
...
throughput: 65{,}313 vs.\ 61{,}761 bottles, a gain of \textbf{3{,}551 bottles per
run} ($+\SI{5.8}{\percent}$, 95\% CI $[2{,}206,\,4{,}896]$). It is the only lever
that significantly raises shipped output, where no capacity lever does, and it
needs only the sensor retrofit, not added line capacity.
