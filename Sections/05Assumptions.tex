...
scraps the whole batch. Premix failure is represented solely through this
probabilistic batch-scrap; the resource-pool breakdown/repair mechanism is
configured but records zero repair events in every run, so the fitted repair
distributions (\S\ref{sec:input}) are not exercised in the baseline and do not
affect any result. CoreTab skips coating, while PlusTab and MaxTab are coated. Packaging is rate-based at \SI{30000}{tablets/hour} per line, and
