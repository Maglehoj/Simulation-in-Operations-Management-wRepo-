...
scraps the whole batch. The same premix failure frequency also drives blender
downtime through the fitted repair time, so scrap and downtime are modelled as
two independent effects rather than one coupled event. This slightly overstates
premix disruption, but it does not affect the bottleneck, flow time or
throughput, because the blenders run far below saturation
($\approx\SI{20}{\percent}$ to \SI{30}{\percent} utilisation). CoreTab skips
coating, while PlusTab and MaxTab are coated. Packaging is rate-based at \SI{30000}{tablets/hour} per line, and
