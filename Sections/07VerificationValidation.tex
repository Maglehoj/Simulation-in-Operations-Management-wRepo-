...
\subsection{Verification (building the model right)}
...
\subsection{Validation and calibration (building the right model)}
...
ordering: drying \SI{94.3}{\percent}, cleaning team \SI{79.6}{\percent} and
packaging \SI{52.4}{\percent}. The per-station mean processing times also
...
\midrule
Batch conservation   & in $=$ shipped $+$ scrapped $=50$ & $46.5 + 3.5 = 50.0$ & \checkmark \\
Primary constraint   & drying (largest unit-h)           & drying \SI{94.3}{\percent} util & \checkmark \\
Constraint ordering  & drying $>$ clean $>$ packaging     & $94.3 > 79.6 > 52.4$ & \checkmark \\
Little's Law         & $\overline{\text{WIP}} \approx \lambda\overline{W}$ & $19.25$ vs.\ $19.43$ (\SI{0.9}{\percent}) & \checkmark \\
\bottomrule
\end{tabularx}
\end{table}
...
count over the makespan: $\lambda = 46.5/433.4 = 0.1073$ batches/h. With
$\overline{W}=\SI{179.4}{\hour}$ this
predicts $\lambda\overline{W}=19.25$ batches, against a directly measured
$\overline{\text{WIP}}=19.43$ batches, a \SI{0.9}{\percent} discrepancy that is
