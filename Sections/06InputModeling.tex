...
\midrule
Drying time (h/batch), $n{=}300$      & Normal & mean $4.00$, std $0.39$ (KS $p=0.92$) & \texttt{normal(0.39, 4.0)} \\
...
of \SI{4.00}{h} (skewness $\approx 0$). The model implements a
\texttt{Normal}$(4.00,0.39)$ distribution, which gives the closest fit to the
symmetric shape (Fig.~\ref{fig:fits}, KS $p=0.92$) and clearly outperforms a
triangular over the observed range $[2.703,5.541]$~h. Because drying is the
bottleneck, throughput is driven by the \emph{mean} (\SI{4.00}{h}); the fitted
mean lies more than ten standard deviations above zero, so the rare possibility
of a non-positive Normal sample is immaterial in practice.
...
The two repair distributions in Table~\ref{tab:fits} were fitted as the case
requires, but the resource-pool breakdown mechanism records zero events in the
runs, so they are documented for completeness and do not influence the reported
results; premix failure enters the model only through the probabilistic
batch-scrap.
...
For drying, the implemented Normal fit (KS $p=0.92$) is overlaid on the
histogram.}
\label{fig:fits}
\end{figure}
