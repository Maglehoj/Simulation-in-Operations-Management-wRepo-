...
We size $n$ with the half-width method~\cite{rossetti2016}. For a KPI with sample mean $\bar x$ and
...
of the mean. We size on mean flow time, the noisiest of the reported
...
\midrule
Mean flow time (h)         & 179.4  & 6.95   & 3.25   & \SI{1.8}{\percent} & 3 \\
Completion / makespan (h)  & 433.4  & 13.58  & 6.36   & \SI{1.5}{\percent} & 2 \\
Throughput (bottles)       & 60{,}494 & 1{,}897 & 888 & \SI{1.5}{\percent} & 2 \\
Avg.\ WIP (batches)        & 19.4   & 0.67   & 0.32   & \SI{1.6}{\percent} & 2 \\
Dryer utilisation          & 0.943  & 0.009  & 0.004  & \SI{0.4}{\percent} & 1 \\
Scrap rate                 & 0.070  & 0.029  & 0.013  & \SI{19}{\percent}  & 5\textsuperscript{*} \\
\bottomrule
\end{tabularx}
\end{table}
...
half-width of $\pm 0.03$, which requires $n=5$. The achieved absolute half-width at
$n=20$ is $\pm 0.013$ (i.e.\ $\pm 1.3$ percentage points).

\paragraph{Chosen replication count.}
Every reported KPI needs at most five replications to reach the target precision, so
...
the one large effect, the Industry 4.0 scrap reduction, while the effect of any
single capacity change on throughput falls below the detectable threshold. As
